...
-- Criando a Stored Procedure P_ClassificacaoGrupoL --
Create Or Alter Procedure P_ClassificacaoGrupoL
As
Begin
...
-- Limpando os Dados da Tabela GrupoL --
Update GrupoL
Set Pontos = 0, Vitorias = 0, Empates=0, Derrotas=0, GolsPro=0, GolsContra=0

-- Atualizando a Tabela de Classificação GrupoL --
...
While @ContadorDeJogos <=6
 Begin
   ...
   Set @CodigoSelecao1 = (Select CodigoSelecao1 From Jogos Where CodigoGrupoSorteio = 12 And CodigoJogoNoGrupo = @ContadorDeJogos)
   Set @CodigoSelecao2 = (Select CodigoSelecao2 From Jogos Where CodigoGrupoSorteio = 12 And CodigoJogoNoGrupo = @ContadorDeJogos)
   Set @GolsSelecao1 = (Select GolsSelecao1 From Jogos Where CodigoGrupoSorteio = 12 And CodigoJogoNoGrupo = @ContadorDeJogos)
   Set @GolsSelecao2 = (Select GolsSelecao2 From Jogos Where CodigoGrupoSorteio = 12 And CodigoJogoNoGrupo = @ContadorDeJogos)	
...
-- Atualizando a Classificação Final definindo a posição de cada Seleção no GrupoL --
;With ClassificacaoFinalGrupoL (ClassificacaoFinal, PosicaoGrupoSorteio, CodigoSelecaoSorteio)
As
(
...
From GrupoL G Inner Join ClassificacaoFinalGrupoL C
                          On G.PosicaoGrupoSorteio = C.PosicaoGrupoSorteio

-- Consultando --
Select Replicate('--',ClassificacaoFinalGrupo)+'>' As 'Grupo L',
