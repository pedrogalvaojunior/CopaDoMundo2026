...
-- Criando a Stored Procedure P_ClassificacaoGrupoI --
Create Or Alter Procedure P_ClassificacaoGrupoI
As
Begin
...
-- Limpando os Dados da Tabela GrupoI --
Update GrupoI
Set Pontos = 0, Vitorias = 0, Empates=0, Derrotas=0, GolsPro=0, GolsContra=0

-- Atualizando a Tabela de Classificação GrupoI --
...
While @ContadorDeJogos <=6
 Begin
   ...
   Set @CodigoSelecao1 = (Select CodigoSelecao1 From Jogos Where CodigoGrupoSorteio = 9 And CodigoJogoNoGrupo = @ContadorDeJogos)
   Set @CodigoSelecao2 = (Select CodigoSelecao2 From Jogos Where CodigoGrupoSorteio = 9 And CodigoJogoNoGrupo = @ContadorDeJogos)
   Set @GolsSelecao1 = (Select GolsSelecao1 From Jogos Where CodigoGrupoSorteio = 9 And CodigoJogoNoGrupo = @ContadorDeJogos)
   Set @GolsSelecao2 = (Select GolsSelecao2 From Jogos Where CodigoGrupoSorteio = 9 And CodigoJogoNoGrupo = @ContadorDeJogos)	
...
-- Atualizando a Classificação Final definindo a posição de cada Seleção no GrupoI --
;With ClassificacaoFinalGrupoI (ClassificacaoFinal, PosicaoGrupoSorteio, CodigoSelecaoSorteio)
As
(
...
From GrupoI G Inner Join ClassificacaoFinalGrupoI C
                          On G.PosicaoGrupoSorteio = C.PosicaoGrupoSorteio

-- Consultando --
Select Replicate('--',ClassificacaoFinalGrupo)+'>' As 'Grupo I',
