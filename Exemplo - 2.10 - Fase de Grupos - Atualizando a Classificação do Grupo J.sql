...
-- Criando a Stored Procedure P_ClassificacaoGrupoJ --
Create Or Alter Procedure P_ClassificacaoGrupoJ
As
Begin
...
-- Limpando os Dados da Tabela GrupoJ --
Update GrupoJ
Set Pontos = 0, Vitorias = 0, Empates=0, Derrotas=0, GolsPro=0, GolsContra=0

-- Atualizando a Tabela de Classificação GrupoJ --
...
While @ContadorDeJogos <=6
 Begin
   ...
   Set @CodigoSelecao1 = (Select CodigoSelecao1 From Jogos Where CodigoGrupoSorteio = 10 And CodigoJogoNoGrupo = @ContadorDeJogos)
   Set @CodigoSelecao2 = (Select CodigoSelecao2 From Jogos Where CodigoGrupoSorteio = 10 And CodigoJogoNoGrupo = @ContadorDeJogos)
   Set @GolsSelecao1 = (Select GolsSelecao1 From Jogos Where CodigoGrupoSorteio = 10 And CodigoJogoNoGrupo = @ContadorDeJogos)
   Set @GolsSelecao2 = (Select GolsSelecao2 From Jogos Where CodigoGrupoSorteio = 10 And CodigoJogoNoGrupo = @ContadorDeJogos)	
...
-- Atualizando a Classificação Final definindo a posição de cada Seleção no GrupoJ --
;With ClassificacaoFinalGrupoJ (ClassificacaoFinal, PosicaoGrupoSorteio, CodigoSelecaoSorteio)
As
(
...
From GrupoJ G Inner Join ClassificacaoFinalGrupoJ C
                          On G.PosicaoGrupoSorteio = C.PosicaoGrupoSorteio

-- Consultando --
Select Replicate('--',ClassificacaoFinalGrupo)+'>' As 'Grupo J',
